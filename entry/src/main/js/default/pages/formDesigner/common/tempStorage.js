 ...
 */
        ...
        async submitUpdate() {
            ...
            for (let index in this.widgetList) {
                ...
                //                查看新旧标签
                //                获取V2VK的唯一化格式，若是旧标签则是旧值，否则V===VK
                let uniVkValue = await this.selectVkByValue(Vk);
                let sameValueVk = await this.selectVkByValue(value);
                console.info("@@@@@@@@@@@uniVk:" + uniVkValue + "~~~~~~~~~~~~~sameValueVk:" + sameValueVk)
                if (Vk === uniVkValue) {
                    console.info("@@@@@@@@@@@new item storage")
                    //                    删除value为空的
                    if (value === "") {
                        //                        删除label，v2vk,vk2v
                        this.deleteV2Vk(Vk)
                        ...
                        if (sameValueVk) {
                            //                        存在旧值
                            console.info("@@@@@@@@@@@old value!")
                            await this.updateLabel(label, sameValueVk)
                            this.deleteV2Vk(Vk)
                            this.deleteVK2V(Vk)
                        } else {
                            console.info("@@@@@@@@@@@storage new value!")
                            //                        不存在
                            //                        复原V2VK
        ...
        async endurance() {
            await  this.storages.labelStorage.flush().then(_ => {
                console.info("~~~~~~~~~~~~~~~~~~storage LABEL success")
            }).catch(err => {
                console.error("!!!!!!!!!!!!!!!!" + err + "!!!!!!!!!!!!!!!!")
            });
            await this.storages.Vk2VStorage.flush().then(_ => {
                console.info("~~~~~~~~~~~~~~~~~~storage VK2V success")
            }).catch(err => {
                console.error("!!!!!!!!!!!!!!!!" + err + "!!!!!!!!!!!!!!!!")
            });
            await this.storages.V2VkStorage.flush().then(_ => {
                console.info("~~~~~~~~~~~~~~~~~~storage V2VK success")
            }).catch(err => {
                console.error("!!!!!!!!!!!!!!!!" + err + "!!!!!!!!!!!!!!!!")
            });

//            this.storages.labelStorage.clear()
//            this.storages.V2VkStorage.clear()
//            this.storages.Vk2VStorage.clear()
        ...
        addLabel(key, value) {
            //            防止同名表项，同步放入！
            this.storages.labelStorage.put(key, value)
        },
        //            ------------------- delete  -------------------
        deleteLabel(key) {
            this.storages.labelStorage.delete(key).then().catch(err => {
                console.error("!!!!!!!!!!!!!!!!" + err + "!!!!!!!!!storage delete label!!!!!!!")
            })
        },
        //            ^^^^^^^^^^^^^^^^^^^ update  ^^^^^^^^^^^^^^^^^^^
        ...
        //            +++++++++++++++++++ add +++++++++++++++++++
        addVk2V(key, value) {
            this.storages.Vk2VStorage.put(key, value)
        },
        //            ------------------- delete  -------------------
        deleteVK2V(key) {
            this.storages.V2VkStorage.delete(key).then().catch(err => {
